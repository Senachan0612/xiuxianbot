...
from . import zmdy_monitor as monitor
from . import zmdy_timing as timing

command = on_command("宗门丹药", aliases={"宗门丹药", "zmdy"}, rule=to_me(), priority=60, block=True)
exit_command = on_command("关闭宗门丹药", aliases={"!宗门丹药", "!zmdy"}, rule=to_me(), priority=60, block=True)
...
async def _(event: GroupMessageEvent, msg: Message = CommandArg()):
    ...
    reply = Message(f"[CQ:at,qq={BotId}]") + Message(f" 宗门丹药领取")
    ...
    while True:
        ...
        went_await = loop.add(loop.loop_await_cmd('宗门丹药', monitor=monitor))
        went_send = loop.add(loop.loop_send_cmd('宗门丹药', cmd=command, msg=reply))
